...
docs/custom-domain.md      # 自定义域名与 SITE_URL 配置
.github/workflows/health.yml  # 每日健康检查
```
...
- 阶段 4 ✅ 运营（自定义域名、监控告警）
  - ✅ 网站 `/api/health` + Sync `/health` 分级状态（healthy / degraded / unhealthy）
  - ✅ `npm run health:check` 与 GitHub Actions 每日巡检
  - ✅ Sync Worker `ALERT_WEBHOOK_URL` 告警（同步失败 / 数据质量降级）
  - ✅ 同步日志状态修正（success / partial / failed）
  - ✅ `docs/custom-domain.md` 自定义域名指南
  - ✅ 修复阶段 3：估分助手 2024 分制链接分数区间错误
