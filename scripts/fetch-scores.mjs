...
async function main() {
  ...
  for (const source of SOURCES) {
    try {
      ...
      succeeded.push({ id: source.id, url: source.url, count });
  ...
  const manifest = {
    ...
    schoolCount: Object.keys(merged).length,
    fetchSources: succeeded.map((s) => s.id),
    autoSync: true,
  };
  fs.writeFileSync(MANIFEST, JSON.stringify(manifest, null, 2), "utf-8");

  console.log(`\n抓取完成: ${Object.keys(merged).length} 所学校有远程数据`);
  if (failed.length > 0) {
    console.log(`部分源失败 ${failed.length} 个，已保留历史数据`);
  }
}
